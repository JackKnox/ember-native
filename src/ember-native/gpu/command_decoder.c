...
b8 emnat_poll_command_buffer(
    ...
    u64 buf_size = command_buf->buffer_size;
    u64 offset = 0;

    if (out_cursor->raw_ptr != NULL) {
        /* raw_ptr points at the payload; the header sits just before it. */
        u64 current = (u64)((u8*)out_cursor->raw_ptr - base) - sizeof(cmd_header);
        offset = align_command(current + out_cursor->size, 16);
    }

    if (offset > buf_size || buf_size - offset < sizeof(cmd_header))
        return EMFALSE;

    cmd_header* hdr = (cmd_header*)(base + offset);
    if (hdr->size < sizeof(cmd_header) || hdr->size > buf_size - offset)
        return EMFALSE;

    out_cursor->raw_ptr = base + offset + sizeof(cmd_header);
